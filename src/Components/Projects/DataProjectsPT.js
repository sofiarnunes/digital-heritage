import CasaGuerrero from '../../Assets/img/projects/casa-guerrero-1.png';
import CasaGuerrero2 from '../../Assets/img/projects/casa-guerrero-panel.jpg';
import CasaGuerrero3 from '../../Assets/img/projects/guerrero-1.jpg';
import CasaGuerrero4 from '../../Assets/img/projects/guerrero-2.jpg';
import CasaGuerrero5 from '../../Assets/img/projects/guerrero-3.jpg';
import CasaGuerrero6 from '../../Assets/img/projects/casa-guerrero-mob-1.jpg';
import CasaGuerrero7 from '../../Assets/img/projects/casa-guerrero-mob-2.jpg';
import CasaGuerrero8 from '../../Assets/img/projects/casa-guerrero-mob-3.jpg';
import CasaGuerrero9 from '../../Assets/img/projects/casa-guerrero-mob-4.jpg';
import CasaGuerrero10 from '../../Assets/img/projects/casa-guerrero-mob-5.jpg';
import CasaGuerrero11 from '../../Assets/img/projects/casa-guerrero-mob-6.jpg';
import RomanMural from '../../Assets/img/projects/roman-mural-1.png';
import RomanMural2 from '../../Assets/img/projects/roman-mural-2.png';
import RomanMural3 from '../../Assets/img/projects/mural-panel.jpg';
import RomanMural4 from '../../Assets/img/projects/mural-1.gif';
import RomanMural6 from '../../Assets/img/projects/mural-mob-1.jpg';
import RomanMural7 from '../../Assets/img/projects/mural-mob-2.jpg';
import RomanMural8 from '../../Assets/img/projects/mural-mob-3.jpg';
import RomanMural9 from '../../Assets/img/projects/mural-mob-4.jpg';
import RomanMural10 from '../../Assets/img/projects/mural-mob-5.jpg';
import RomanMural11 from '../../Assets/img/projects/mural-mob-6.jpg';
import LowRelief from '../../Assets/img/projects/low-relief-1.png';
import LowRelief2 from '../../Assets/img/projects/low-relief-2.png';
import LowRelief3 from '../../Assets/img/projects/relief-panel.jpg';
import LowRelief4 from '../../Assets/img/projects/relief-mob-1.jpg';
import LowRelief5 from '../../Assets/img/projects/relief-mob-2.jpg';
import LowRelief6 from '../../Assets/img/projects/relief-mob-3.jpg';
import LowReliefv1 from '../../Assets/img/projects/Complex/Relief/relief-v1.jpg';
import LowReliefh1 from '../../Assets/img/projects/Complex/Relief/relief-h1.jpg';
import LowReliefv2 from '../../Assets/img/projects/Complex/Relief/relief-v2.mp4';
import LowReliefh2 from '../../Assets/img/projects/Complex/Relief/relief-h2.mp4';
import LowReliefv3 from '../../Assets/img/projects/Complex/Relief/relief-v3.jpg';
import LowReliefh3 from '../../Assets/img/projects/Complex/Relief/relief-h3.mp4';
import TouchCollection1 from '../../Assets/img/projects/collection-1.jpg';
import TouchCollection2 from '../../Assets/img/projects/collection-panel.jpg';
import TouchCollection3 from '../../Assets/img/projects/collection-mob-1.jpg';
import TouchCollection4 from '../../Assets/img/projects/collection-mob-2.jpg';
import TouchCollection5 from '../../Assets/img/projects/collection-mob-3.jpg';
import Touchv1 from '../../Assets/img/projects/Complex/Collection/collection-v1.JPG';
import Touchh1 from '../../Assets/img/projects/Complex/Collection/collection-h1.jpeg';
import Touchv2 from '../../Assets/img/projects/Complex/Collection/collection-v2.mp4';
import Touchh2 from '../../Assets/img/projects/Complex/Collection/collection-h2.png';
import Touchv3 from '../../Assets/img/projects/Complex/Collection/collection-v3.JPG';
import Touchh3 from '../../Assets/img/projects/Complex/Collection/collection-h3.mp4';



const DataProjectsPT = [
    {
        name: 'Digitalização e impressão 3D para coleção de toque',
        name_1: 'touch collection',
        name_11: 'coleção de toque',
        type: '3',
        format: 'extended',
        link: '/projects/digitization-3Dprinting-for-touch-collection',
        date_begin: 'Dezembro, 2021',
        date_end: 'presente',
        img_2: TouchCollection1,
        img_3: TouchCollection2,
        img_4: TouchCollection3,
        img_5: TouchCollection4,
        img_6: TouchCollection5,
        v1: Touchv1,
        h1: Touchh1,
        v2: Touchv2,
        h2: Touchh2,
        v3: Touchv3,
        h3: Touchh3,
        title_1: 'Digitalização e modelação 3D',
        description_1: 'Um conjunto de obras representativas de diferentes coleções do Museu Calouste Gulbenkian foi selecionado para criar uma Coleção de Toque, impressa em 3D, para públicos invisuais. Primeiramente, os modelos 3D das obras de arte foram criados por meio da fotogrametria digital e modelação 3D.',
        title_2: 'Modelos interativos',
        description_2: 'Os modelos 3D foram desenvolvidos para abrir e fechar as suas portas e gavetas, como os objetos originais, para garantir uma compreensão mais completa dessas obras e uma interatividade semelhante à original.',
        title_3: 'Impressão 3D',
        description_3: 'Os modelos serão impressos em 3D para serem tocados pelos públicos invisuais, durante a exposição no museu.',
    },
    {
        name: 'Restauro virtual de um baixo relevo egípcio (300-250 aC)',
        name_1: 'Egyptian low relief (300-250 BC)',
        name_11: 'baixo relevo egípcio (300-250 aC)',
        link: '/projects/virtual-restoration-egyptian-low-relief',
        type: '1',
        format: 'extended',
        date_begin: 'Setembro, 2021',
        date_end: 'Novembro, 2021',
        img_1: LowRelief,
        img_2: LowRelief2,
        img_3: LowRelief3,
        img_4: LowRelief4,
        img_5: LowRelief5,
        img_6: LowRelief6,
        v1: LowReliefv1,
        h1: LowReliefh1,
        v2: LowReliefv2,
        h2: LowReliefh2,
        v3: LowReliefv3,
        h3: LowReliefh3,
        title_1: 'Modelo 3D',
        description_1: 'Pensa-se que este baixo-relevo egípcio (300-250 aC), pertencente ao Museu Calouste Gulbenkian, é um estudo de artista, em tempos pintado. Um modelo 3D desta obra de arte foi primeiramente criado, usando a fotogrametria digital.',
        title_2: 'Restauro Virtual',
        description_2: 'Danos como lacunas, intervenções anteriores de conservação, marcas de abrasão, fungos e outras manchas foram identificados na obra. Esta avaliação de conservação informou sobre o restauro virtual do gémeo digital.',
        title_3: 'Reconstrução das cores',
        description_3: 'Foi realizado um estudo exaustivo das cores mais utilizadas em pinturas murais, na época. Também foi feito um estudo de paralelos para melhor apoiar as escolhas de cores e as suas diferentes disposições.',
    },
    {
        name: 'Restauro virtual de uma pintura mural romana',
        name_1: 'roman mural painting',
        name_11: 'pintura mural romana',
        type: '1',
        format: 'simple',
        link: '/projects/virtual-restoration-roman-mural-painting',
        date_begin: 'Julho, 2021',
        date_end: 'Agosto, 2021',
        img_1: RomanMural,
        img_2: RomanMural2,
        img_3: RomanMural3,
        img_4: RomanMural4,
        img_5: RomanMural6,
        img_6: RomanMural7,
        img_7: RomanMural11,
        img_8: RomanMural8,
        img_9: RomanMural9,
        img_10: RomanMural10,
        img_11: RomanMural10,
        img_12: RomanMural10,
        description: 'Um termopólio, coberto de belos frescos, foi encontrado recentemente (2020) no Beco das Varandas, em Pompeia. As extensas deformações no fresco, causadas pela erupção vulcânica, foram recuperadas digitalmente, permitindo uma percepção completa da mesma imagem de há milhares de anos atrás.',
    },
    {
        name: 'Modelação 3D da Casa Guerrero',
        name_1: 'Casa Guerrero',
        name_11: 'Casa Guerrero',
        type: '3',
        format: 'simple',
        link: '/projects/3D-modelling-casa-guerrero',
        date_begin: 'Maio, 2021',
        date_end: 'Junho, 2022',
        img_1: CasaGuerrero,
        img_2: CasaGuerrero,
        img_3: CasaGuerrero2,
        img_4: CasaGuerrero3,
        img_5: CasaGuerrero4,
        img_6: CasaGuerrero5,
        img_7: CasaGuerrero6,
        img_8: CasaGuerrero7,
        img_9: CasaGuerrero8,
        img_10: CasaGuerrero9,
        img_11: CasaGuerrero10,
        img_12: CasaGuerrero11,
        description: 'Utilizando plantas CAD da Casa Guerrero, do arquiteto Alberto Campo Baeza, em Cádis, Espanha, foi criado um modelo 3D deste espaço arquitetónico. O entorno também foi considerado na criação de um ambiente fotorrealista, com a modelação de água, árvores e outros elementos vegetalistas.',
    },
]


export default DataProjectsPT